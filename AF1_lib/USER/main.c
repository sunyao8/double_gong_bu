...
#define current_capacitor_TASK_PRIO       			7
//设置任务堆栈大小
#define Current_Capacitor_SIZE  		    		128
//任务堆栈
OS_STK Current_Capacitor_TASK_STK[Current_Capacitor_SIZE];
//任务函数
void current_capacitor_task(void *pdata);
...
extern s8 L_C_flag;
extern vu8 tempshuzhi,tempshuzhi2;
extern u8 slave[33];
...
void current_capacitor_AC (void);
...
void current_capacitor_task(void *pdate)
{
while(1)
{
if(mybox.master==0) current_capacitor_AC ();
...
/************从机功能 温度报警************************/
{
if((tempshuzhi>=temperature_gate||tempshuzhi2>=temperature_gate)&&temperature_warn==0&&auto_on==1)
{
GPIO_SetBits(GPIOA,GPIO_Pin_0);
...
if(tempshuzhi<=(temperature_gate-6)&&tempshuzhi2<=(temperature_gate-6)&&temperature_warn==1)
{
	 temperature_warn=0;
	  rework_time[1]=1;//向time3定时器打开放电时间
 rework_time[0]=1;//向time3定时器打开放电时间
