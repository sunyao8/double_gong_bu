...
void Graf_C_cuirrent(u32 current,u8 AC)
{
   u32 temp;
   u16 currshiwei,currgewei,currshifenwei,currbaifenwei,currqianfenwei;
   ...
   if(auto_on==0)
	...
	if(AC==A)	Write_1621(4,0x04);   //显示 前A
	if(AC==B)	Write_1621(4,0x02);   //显示 前B
	if(AC==C)	Write_1621(4,0x01);   //显示 前C
...
void Graf_temp(u8 temp,u8 temp2)
{
	u8 tempbaiwei,tempshiwei,tempgewei;
	u8 tempbaiwei2,tempshiwei2,tempgewei2;

	Write_1621(14,0x04);	//显示TEMP1
		Write_1621(12,0x08);	//显示TEMP  T2
   ...
   if(auto_on==0)
	...
	tempbaiwei=temp/100;
	WriteAll_1621(16,num12345Seg+2*tempbaiwei,2);	//显示TEMP百数位

	tempshiwei=(temp%100)/10;
	WriteAll_1621(18,num12345Seg+2*tempshiwei,2);	//显示TEMP十数位

	tempgewei=temp%10;
	WriteAll_1621(20,num12345Seg+2*tempgewei,2);	//显示TEMP个数位

		tempbaiwei2=temp2/100;
	WriteAll_1621(2,num12345Seg+2*tempbaiwei2,2);	  //显示volt百位部分

	
	tempshiwei2=(temp2%100)/10;
	WriteAll_1621(10,num67Seg+2*tempshiwei2,2);	  //显示volt十位部分

	tempgewei2=temp2%10;
	WriteAll_1621(8,num67Seg+2*tempgewei2,2);	  //显示volt个位部分
