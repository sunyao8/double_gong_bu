...
//Ä£Ê½¿ØÖÆ
 vu16  dog_clock=20;
		 u16 K_BT=10;//µçÁ÷ÏµÊı

 OS_EVENT * RS485_MBOX,* RS485_STUTAS_MBOX,* RS485_RT;			//	rs485ÓÊÏäĞÅºÅÁ¿
 OS_EVENT *Heartbeat;			 //ĞÄÌøĞÅºÅÁ¿
OS_EVENT *master_led_task;
//OS_EVENT * sub_machine1_open;		//ÏÂÎ»»úÃüÁîĞÅºÅ
//OS_EVENT * sub_machine1_close;		//ÏÂÎ»»úÃüÁîĞÅºÅ
//OS_EVENT * sub_machine2_open;		//ÏÂÎ»»úÃüÁîĞÅºÅ
//OS_EVENT * sub_machine2_close;		//ÏÂÎ»»úÃüÁîĞÅºÅ

OS_EVENT *scan_slave;

vu8 cont=0;//ÓÃÓÚ¸ü¸ÄÖ÷»úºÅµÄ¼Ç´ÎÊıÆ÷
u32 life_time_1=0;//´Ó»ú1¹¤×÷Ê±¼äµÄÖĞ¼ä±äÁ¿
u32 life_time_2=0;//´Ó»ú2¹¤×÷Ê±¼äµÄÖĞ¼ä±äÁ¿
u32 idle_time=0;//Ö÷»úÓÃÓÚÂÖĞİµÄÊ±¼ä
u32 scan_time=0;//Ö÷»úÓÃÓÚ´ÓĞÂ±éÀú´Ó»úµÄµÄÊ±¼ä
...
vu8 rs485buf[LEN_control];//·¢ËÍ¿ØÖÆĞÅÏ¢
vu8 statusbuf[LEN_status];//·¢ËÍ×´Ì¬ĞÅÏ¢
 ...
 void TIM3_Int_Init(u16 arr,u16 psc)
{
	...
	TIM_ITConfig(TIM3,TIM_IT_Update,ENABLE ); //Ê¹ÄÜÖ¸¶¨µÄTIM4ÖĞ¶Ï,ÔÊĞí¸üĞÂÖĞ¶Ï

	//ÖĞ¶ÏÓÅÏÈ¼¶NVICÉèÖÃ
	NVIC_InitStructure.NVIC_IRQChannel = TIM3_IRQn;  //TIM4ÖĞ¶Ï
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;  //ÏÈÕ¼ÓÅÏÈ¼¶0¼¶
 ...
 void TIM3_IRQHandler(void)   //TIM4ÖĞ¶Ï
{	 static u8 count_rework[2];
	OSIntEnter();   
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)  //¼ì²éTIM4¸üĞÂÖĞ¶Ï·¢ÉúÓë·ñ
		{	  
...
/***************************************************************/			
		...
		if(RS485_RX_BUF[1]=='#')
			{
/*
				if(mybox.master==1)//Èç¹ûÊÇÖ÷»ú£¬¼ì²é´Ó»úÖĞÓĞÎŞidºÅ±ÈÖ÷»úĞ¡µÄ£¬Èç¹ûÓĞ½»»»Ö÷´Ó»ú
					{
				...
				if(mybox.myid>RS485_RX_BUF[2])
			   	...
			   	{
			   	/*
			   	if(mybox.master==1)//Èç¹ûÊÇÖ÷»ú£¬¼ì²éÆäËûÓĞÎŞÖ÷»úÖĞidºÅ±È±¾Ö÷»úĞ¡µÄ£¬Èç¹ûÓĞ½»»»Ö÷´Ó»ú
					{
...
/************************·Ö²¹ÊÕ¼¯×´Ì¬Ğ­Òé£¬¹²²¹ÏµÍ³ ÊÕµ½ºóÉáÆú ²¢Çå¿ÕRS485_RX_CNT ***********************************/		
			if(RS485_RX_BUF[RS485_RX_CNT-1]=='%'){RS485_RX_BUF[0]='%'; RS485_RX_CNT=1;}
		if(RS485_RX_BUF[RS485_RX_CNT-1]==')')
	{
				RS485_RX_CNT=0;
...
/*************************·Ö²¹ÏµÍ³ÏÔÊ¾Êı¾İĞ­Òé£¬¹²²¹ÏµÍ³ ÊÕµ½ºóÉáÆú ²¢Çå¿ÕRS485_RX_CNT **********************************/		
		...
		if(RS485_RX_BUF[RS485_RX_CNT-1]=='=')
		{			RS485_RX_CNT=0;
/***********************Èç¹ûÓĞ·Ö²¹»úÆ÷£¬¹²²¹Èç¹ûÊÇÖ÷»ú£¬¾ÍÒª½øĞĞÇĞ»»Ö÷»ú************************/
				if(mybox.master==1)
					{
					mybox.master=0;
					     hguestnum=222;
...
/*****************************Èç¹ûÓĞ·Ö²¹»úÆ÷£¬¹²²¹Èç¹ûÊÇÖ÷»ú£¬¾ÍÒª½øĞĞÇĞ»»Ö÷»úend******************************************/
...
void turn_master_id(u8 id)//¸Ä±äµ±Ç°Õû¸öÏµÍ³ÖĞÖ÷»úµÄIDºÅ
{
 ...
 int rs485_trans_order(vu8 *tx_r485)//½âÎöÓÉÖ÷»ú·¢ËÍ¹ıÀ´µÄĞÅºÅ£¬²¢·¢ËÍ¸øÏÂÎ»»ú
{
...
return 0;
 ...
 void order_trans_rs485(u8 source,u8 destination, u8 send,u8 relay,u8 message,u8 ctr)//Ö÷»ú³ÌĞò£¬Ö÷»úÃüÁî½âÎö³ÉRS485ĞÅÏ¢£¬·¢ËÍ¸øÄ¿µÄ´Ó»ú
{   OS_CPU_SR cpu_sr=0;
   OS_ENTER_CRITICAL();
  ...
  if(ctr==CONTROL)
	...
	RS485_Send_Data(rs485buf,10);//·¢ËÍ5¸ö×Ö½Ú
	   	if(destination==source){mybox.send=send;slave_control(relay, message);}//Èç¹ûĞÅÏ¢·¢¸øµÄ×Ô¼º
...
void led_on_off(u8 on_off,u8 j) //²ÎÊıÖµ1 Îª´ò¿ªled £¬0Îª¹Ø±Õled
{
}
...
{  	 OS_CPU_SR cpu_sr=0;
	...
	statusbuf[12]='*';
	RS485_Send_Data(statusbuf,13);//·¢ËÍ10¸ö×Ö½Ú
	OS_EXIT_CRITICAL();	
}
 void status_trans_rs485_scantask(status_box *mystatus)//´Ó»ú³ÌĞò
{  	 OS_CPU_SR cpu_sr=0;
    OS_ENTER_CRITICAL();
    statusbuf[0]='&';
	statusbuf[1]='#';
 ...
 void status_trans_rs485_dis(status_box *mystatus)//´Ó»ú³ÌĞò
{  	 OS_CPU_SR cpu_sr=0;
    OS_ENTER_CRITICAL();
    statusbuf[0]='&';
	statusbuf[1]='#';
 ...
 void status_trans_rs485_RT()//´Ó»ú³ÌĞò
{  	 OS_CPU_SR cpu_sr=0;
    OS_ENTER_CRITICAL();
    statusbuf[0]='&';
	statusbuf[1]='+';
 ...
 void status_trans_rs485_comm_RT()//´Ó»ú³ÌĞò
{  	 OS_CPU_SR cpu_sr=0;
    OS_ENTER_CRITICAL();
    statusbuf[0]='&';
	statusbuf[1]='!';
...
u8 sort_idlenode_list(idle_list *sort_idle_list,status_list_node *list)//¿ÕÏĞÓĞĞò¶ÓÁĞ(°´ÈİÁ¿´óĞ¡ÓÉ´óµ½Ğ¡ÅÅÁĞ£¬·µ»Ø¿ÕÏĞ½Úµã¸öÊı)
{
   u8 i,j=1,k,t,flag=0;
   u8 count=0;
...
/**************½«µÚÒ»×é¿ÕÏĞ¶ÓÁĞºÍµÚ¶ş×é¿ÕÏĞ¶ÓÁĞ×é³ÉÒ»×é½øĞĞÅÅĞòÂÖ Ğİ***************************/


u8 turn_idlenode_list(turn_node *turn_idle_list,status_list_node *list_1,status_list_node *list_2)//¿ÕÏĞÓĞĞò¶ÓÁĞ(°´ÈİÁ¿´óĞ¡ÓÉ´óµ½Ğ¡ÅÅÁĞ£¬·µ»Ø¿ÕÏĞ½Úµã¸öÊı)
{
   	              ...
   	              if(list_1[slave[i]].work_status==0&&list_1[slave[i]].size!=0)
                      { turn_idle_list[j].myid=list_1[slave[i]].myid;
						...
						turn_idle_list[j].group=1;
					          j++;
						count++;
						if(flag==0)flag=1;//Èç¹ûÃ»ÓĞ¿ÕÏĞ½Úµã
   	              	  }
                    }
...
*/
u8 sort_busynode_list(busy_list *sort_busy_list,status_list_node *list)//Ã¦ÂµÓĞĞò¶ÓÁĞ(°´¹¤×÷Ê±¼ä´óĞ¡ÓÉ´óµ½Ğ¡ÅÅÁĞ)
{
...
u8 sort_busynode_list_asc(busy_list *sort_busy_list,status_list_node *list)//Ã¦ÂµÓĞĞò¶ÓÁĞ(°´¹¤×÷ÈİÁ¿´óĞ¡ÓÉ´óµ½Ğ¡ÅÅÁĞ)
{
   u8 i,j=1,g,f,k,t,w,flag=0;
   u8 count=0,count_time=0;
   for(i=1;i<=slave[0];i++){sort_busy_list[i].myid=0;sort_busy_list[i].size=0;}
...
/************************************phase******************/
		...
		//	if((angle[3]>-150.0&&angle[3]<-30.0))flag_phase=0;
			...
			{	if(angle[3]>0){while(1){if(angle[3]>360){angle[3]=angle[3]-360;} else break;}}
				...
				if((angle[3]>3.0&&angle[3]<178.0)||(angle[3]>-357.0&&angle[3]<-182.0))flag_phase=1;//ÕıĞò
				
			else if((angle[3]>182.0&&angle[3]<357.0)||(angle[3]>-178.0&&angle[3]<-3.0))flag_phase=0;//·´Ğò
...
void temperature()   //µçÈİÆ÷ÎÂ¶È¼ì²â
{
...
//	  adc_tmp2=Get_Adc_Average(ADC_Channel_6,10);
   ...
   adc_tmp2=Get_Adc2(ADC_Channel_6);
//	  adc_tmp2=Get_Adc_Average(ADC_Channel_6,10);
	  tempshuzhi2=(u8)(258-((adc_tmp2*255)/4096));
...
u8 i=0;
{
  if(size_1==2)
  	{
  	for(i=slave_comm[9];i>slave_comm[8];i--)//20µÄ¶ÓÁĞÒÆ¶¯
  		{
	...
	slave_comm[9]++;
	  	...
	  	for(i=slave_comm[7];i>slave_comm[6];i--)//10µÄ¶ÓÁĞÒÆ¶¯
  		{
	   comm_list[i].myid=comm_list[i-1].myid;
   	   comm_list[i].size=comm_list[i-1].size;
   	   comm_list[i].work_status=comm_list[i-1].work_status;
	   comm_list[i].group=comm_list[i-1].group;
		...
		slave_comm[7]++;
	  	...
	  	for(i=slave_comm[5];i>slave_comm[4];i--)//5µÄ¶ÓÁĞÒÆ¶¯
  		{
	   comm_list[i].myid=comm_list[i-1].myid;
   	   comm_list[i].size=comm_list[i-1].size;
   	   comm_list[i].work_status=comm_list[i-1].work_status;
...
{
slave_comm[2]=0;//¶ÓÁĞ2v ±êÊ¾³õÊ¼»¯
slave_comm[3]=0;

slave_comm[4]=1;//¶ÓÁĞ5v ±êÊ¾³õÊ¼»¯
slave_comm[5]=1;

slave_comm[6]=2;//¶ÓÁĞ10v ±êÊ¾³õÊ¼»¯
slave_comm[7]=2;

slave_comm[8]=3;//¶ÓÁĞ20v ±êÊ¾³õÊ¼»¯
slave_comm[9]=3;
  	...
  	
{
	...
	if(id==comm_list[i].myid&&group==comm_list[i].group)//Ëø¶¨½Úµã½øĞĞ¸üĞÂ
		{ 
...
u8 m,s,w,g;
{
  if(size==2)
	...
	for(i=slave_comm[2];i<slave_comm[3]-1;i++)// 2 µÄ¶ÓÁĞÒÆ¶¯
  		{
	   comm_list[i].myid=comm_list[i+1].myid;
   	   comm_list[i].size=comm_list[i+1].size;
   	   comm_list[i].work_status=comm_list[i+1].work_status;
	...
	for(i=slave_comm[4];i<slave_comm[5]-1;i++)//20µÄ¶ÓÁĞÒÆ¶¯
  		{
	   comm_list[i].myid=comm_list[i+1].myid;
   	   comm_list[i].size=comm_list[i+1].size;
   	   comm_list[i].work_status=comm_list[i+1].work_status;
	...
	for(i=slave_comm[6];i<slave_comm[7]-1;i++)//20µÄ¶ÓÁĞÒÆ¶¯
  		{
	   comm_list[i].myid=comm_list[i+1].myid;
   	   comm_list[i].size=comm_list[i+1].size;
   	   comm_list[i].work_status=comm_list[i+1].work_status;
	...
	for(i=slave_comm[8];i<slave_comm[9]-1;i++)//20µÄ¶ÓÁĞÒÆ¶¯
  		{
	   comm_list[i].myid=comm_list[i+1].myid;
   	   comm_list[i].size=comm_list[i+1].size;
   	   comm_list[i].work_status=comm_list[i+1].work_status;
