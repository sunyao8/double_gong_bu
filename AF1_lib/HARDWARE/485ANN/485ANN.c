...
vu8 cont=0;//ÓÃÓÚ¸ü¸ÄÖ÷»úºÅµÄ¼Ç´ÎÊýÆ÷
...
vu8 rs485buf[LEN_control];//·¢ËÍ¿ØÖÆÐÅÏ¢
vu8 statusbuf[LEN_status];//·¢ËÍ×´Ì¬ÐÅÏ¢
...
{	 static u16 count_rework[2];
 ...
 if(rework_time[0]==1)
 	{
 	count_rework[0]++;
	if(count_rework[0]==1200)//·Åµç 20·ÖÖÓ
		{
count_rework[0]=0;
rework_time[0]=0;
...
{
	c_rework++;
if(c_rework==72*20)//72 ´ÎÎªÒ»·ÖÖÓ
 //if(c_rework==1)//7´ÎÎª6Ãë²âÊÔ
		{
...
void turn_master_id(u8 id)//¸Ä±äµ±Ç°Õû¸öÏµÍ³ÖÐÖ÷»úµÄIDºÅ
{
 ...
 int rs485_trans_order(vu8 *tx_r485)//½âÎöÓÉÖ÷»ú·¢ËÍ¹ýÀ´µÄÐÅºÅ£¬²¢·¢ËÍ¸øÏÂÎ»»ú
{
...
//		id_num=AT24CXX_ReadOneByte(0x0010);//¼ÓÉÏÕâ¾äºó£¬ÉèÖÃ±ä±ÈBTºó£¬idºÅ»á³öÏÖ¹éÁãÎÊÌâ£¬Ð¾Æ¬¹îÒìÎÊÌâ
...
if(dianliuzhi<=K_BT){gonglvshishu=100;dianliuzhi=0;L_C_flag=1;}//µçÁ÷Ð¡ÓÚ0.1A Ê±£¬µçÁ÷¾ÍÇåÁã
...
/*×Ô¶¯ÅÐ±ð±ä±È½øÐÐÏµÍ³¼ÇÒä£¬´æÈëEPROMÀï*/

AT24CXX_WriteOneByte(0x0100,BT_num);					

RT_FLAG=4;//2//2²»ÄÜÓÃÐ¾Æ¬bug
/**************************************ÇÐÖ÷»ú**/
{
 	{
...
void verson_updata()//´Ë¹¦ÄÜ¹ÒÔÚÆÁÄ»ÏÔÊ¾ÈÎÎñÖÐ
{
u8 ver;
u16 time_alive=TIME_LIVE;//Äê* Ìì*Ð¡Ê±* ·ÖÖÓ1*356*24*60
while(AT24CXX_Check());
if(AT24CXX_Check()==0)
{
if(vernum<=23)
...
if(ver>23&&ver!=225)//¿ÉÄÜflashÎª·Ç·¨²Ù×÷£¬Ê±¼ä¹éÁã
{
...
while(1)
{
AT24CXX_WriteOneByte(0x9000,0);//Ð´Èë°æ±¾ºÅ
if(0==AT24CXX_ReadOneByte(0x9000))break;//±£Ö¤Ð´½øÈ¥ÁË
}
}


if((vernum>ver||ver==255)) //¸üÐÂ°æ±¾ »ò ÐÂ»úÆ÷
{
TO=0;
AT24CXX_WriteLenByte_sy(0xe000, time_alive,2);
while(1)
{
AT24CXX_WriteOneByte(0x9000,vernum);//Ð´Èë°æ±¾ºÅ
if(vernum==AT24CXX_ReadOneByte(0x9000))break;//±£Ö¤Ð´½øÈ¥ÁË
}
}
